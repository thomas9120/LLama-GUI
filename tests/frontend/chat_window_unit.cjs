...
function loadApi(t) {
    const api = loadPackage().chatWindow;
    // Each test owns a fresh VM and disposes every coordinator/adapter even
    // when an assertion fails before the scenario's explicit cleanup.
    for (const name of ["createCoordinator", "createHostAdapter"]) {
...
test("package evaluation is inert and JSON projection rejects unsafe values", () => {
...
test("flag bridge reads live settings and isolates subscriptions and failures", () => {
...
test("host bootstrap initializes once and revokes ownership before Chat init", async t => {
    const facade = loadApi(t);
...
test("disposing a queued coordinator preserves the other owner", async t => {
    const api = loadApi(t);
...
test("host projections filter nested fields and return independent copies", async t => {
    const api = loadApi(t);
...
test("adapter subscriptions and authentication respect session validity", async t => {
    const api = loadApi(t);
    let observe;
    let cleanups = 0;
    let authCalls = 0;
...
test("adapter settings preserve unset values and reject forbidden writes", async t => {
    const api = loadApi(t);
    const calls = [];
    const core = {
...
test("unavailable Web Locks retain a usable single-window owner", async t => {
    const api = loadApi(t);
...
test("peer handshake rejects wrong source origin version and epoch", async t => {
    const api = loadApi(t);
    const messages = [];
...
test("an untrustworthy origin rejects every handshake", async t => {
    const api = loadApi(t);
    const messages = [];
...
test("repeated forward and reverse transfers preserve the saved conversation", async t => {
    const api = loadApi(t);
    const pair = makePeerPair(api);
    await preparePair(pair);
...
test("queued receiver commit times out without acquiring ownership later", async t => {
    const api = loadApi(t);
    const pair = makePeerPair(api);
    await preparePair(pair);
...
// Storage quota failures abort before source release and preserve the
// usable owner/history. A receiver that disappears also times out before
// commit, leaving no queued stale lock callback.
test("storage write failure and a missing receiver preserve source ownership", async t => {
    const api = loadApi(t);
...
for (const failure of ["missing recovery storage methods", "blocked recovery storage reads"]) {
    test(`${failure} refuse transfer without losing the usable owner`, async t => {
        const api = loadApi(t);
        const storage = failure.startsWith("missing") ? {} : new FakeStorage();
        storage.failRead = true;
        const pair = makePeerPair(api, { storage });
        await preparePair(pair);
        const before = pair.uiA.snapshotState();

        assert.equal(pair.a.readRecovery().reason, "storage-unavailable");
        assert.equal(await pair.a.beginTransfer({ destinationId: "B" }), false);
        assert.equal(pair.a.isOwner(), true);
        assert.equal(pair.b.isOwner(), false);
        assert.equal(pair.uiA.suspended, false);
        assert.deepEqual(pair.uiA.snapshotState().messages, before.messages);
        assert.equal(pair.uiA.snapshotState().draft, before.draft);
        assert.equal(pair.uiB.restores, 0);
        assert.equal(pair.sent.some(item => item.message.type === "commit"), false);
        assert.equal(pair.locks.queue.length, 0);
    });
}
...
test("receiver disappearance before and after release requires the right recovery", async t => {
    const api = loadApi(t);
...
test("invalidation during prepare restore prevents stale readiness", async t => {
    const api = loadApi(t);
    const pair = makePeerPair(api);
    await preparePair(pair);
    let finishRestore;
...
test("invalidation during commit restore releases the destination lock", async t => {
    const api = loadApi(t);
...
test("delayed ownership acknowledgment requires explicit recovery", async t => {
    const api = loadApi(t);
    let droppedAck = null;
...
test("invalidation while acknowledgment is pending retires the transfer", async t => {
    const api = loadApi(t);
    let droppedAck = null;
...
test("durable deletion tombstones prevent stale history resurrection", async t => {
    const api = loadApi(t);
    const locks = new FakeLocks();
    const storage = new FakeStorage();
...
test("malformed recovery records require explicit recovery", async t => {
    const api = loadApi(t);
    const storage = new FakeStorage();
    const locks = new FakeLocks();
...
test("owner initialization is idempotent and abort rejection is acknowledged", async t => {
    const api = loadApi(t);
    const locks = new FakeLocks();
    const storage = new FakeStorage();
...
test("third-page contention and host revocation fail closed", async t => {
    const api = loadApi(t);
    const pair = makePeerPair(api);
...
test("disposing a receiver during restore prevents late activation", async t => {
    const api = loadApi(t);
    const pair = makePeerPair(api);
    await preparePair(pair);
    let finishRestore;
...
test("host-loss quarantine retains its lock until explicit recovery", async t => {
    const api = loadApi(t);
    const locks = new FakeLocks();
    const storage = new FakeStorage();
