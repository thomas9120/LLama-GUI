...
async function runScenario(browser, port, verify) {
    ...
    try {
        ...
        // Each selected callback runs with this invocation's fresh page and API
        // fixtures. Related steps share state only within their named scenario.
        const scenarios = {
            async "preset imports and safe notifications"() {
            ...
            async "mirrored launch controls and Configure search"() {
                await selectSection(page, "quick-launch");
            ...
            async "inference rates and authenticated polling"() {
                await page.evaluate(() => window.LlamaGui.flagCore.setFlagValue("api_key", "first-secret, second-secret"));
                await selectSection(page, "quick-launch");
                statsMetrics = {
                    promptTokens: 40,
                    promptSpeed: 4,
            ...
            async "failed Stop recovery and independent inference sources"() {
            ...
            async "Chat settings panel responsive collapse"() {
                await selectSection(page, "chat");
            ...
            async "Chat availability and shared request settings"() {
                await page.evaluate(() => window.LlamaGui.flagCore.setFlagValue("api_key", "first-secret, second-secret"));
                await selectSection(page, "chat");
            ...
            async "Chat reasoning persistence and raw thinking tags"() {
                await startChat();
            ...
            async "Chat empty responses and stopped partial streams"() {
                await startChat();
                await page.click("#btn-chat-new");
                await page.evaluate(() => {
                    const originalFetch = window.fetch;
            ...
            async "Chat streaming scroll position and response metadata"() {
                await startChat();
                await page.click("#btn-chat-new");
                await page.evaluate(() => {
                    const originalFetch = window.fetch;
            ...
            async "Chat editing and failed regeneration recovery"() {
                await startChat();
            ...
            async "Chat context preview and overflow recovery"() {
                await startChat();
                await page.click("#btn-chat-new");
            ...
            async "Chat compaction controls and transcript preservation"() {
                await startChat();
            ...
            async "shared sampler controls and preset lifecycle"() {
                await page.evaluate(() => window.LlamaGui.flagCore.setMultipleFlagValues({ ctx_size: 12345, gpu_layers: "9" }));
            ...
            async "external server connection and remembered targets"() {
                await selectSection(page, "api");
            ...
            async "custom backend activation preserves shared flags"() {
                await page.evaluate(() => window.LlamaGui.flagCore.setMultipleFlagValues({ temperature: 0.37, ctx_size: 12345 }));
                await selectSection(page, "install");
            ...
            async "model switcher refresh drag and keyboard controls"() {
                await selectSection(page, "quick-launch");
            ...
            async "preset list roving keyboard focus"() {
            ...
            async "models folder changes refresh shared selection"() {
            ...
            async "Monitor cards output and shared inference baseline"() {
                // ── Monitor tab ─────────────────────────────────────────────────
        ...
        assert.equal(typeof scenarios[verify], "function", `unknown smoke scenario: ${verify}`);
...
for (const [name, verify = name] of [
    ...
    ["mirrored launch controls and Configure search"],
    ["API key controls and masked command previews"],
    ["inference rates and authenticated polling"],
    ["failed Stop recovery and independent inference sources"],
    ["Chat settings panel responsive collapse"],
    ["Chat availability and shared request settings"],
    ["Chat reasoning persistence and raw thinking tags"],
    ["Chat empty responses and stopped partial streams"],
    ["Chat streaming scroll position and response metadata"],
    ["Chat editing and failed regeneration recovery"],
    ["Chat context preview and overflow recovery"],
    ["Chat compaction controls and transcript preservation"],
    ["shared sampler controls and preset lifecycle"],
    ["API endpoints and remote tunnel states"],
    ["external server connection and remembered targets"],
    ["custom launch arguments and environment validation"],
    ["custom backend activation preserves shared flags"],
    ["model switcher refresh drag and keyboard controls"],
    ["preset list roving keyboard focus"],
    ["models folder changes refresh shared selection"],
    ["Monitor cards output and shared inference baseline"],
