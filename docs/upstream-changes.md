...
- **Planned local implementation (after merge, once a bundled/downloaded binary advertises the flag):**
  1. Add `{ value: "draft-mtp-adaptive", label: "Draft MTP (Adaptive Depth)" }` (or similar) to the `spec_type` enum options in `ui/js/flags/definitions-speculative.js`; it shares the existing `--spec-type` flag and the ngram-mod stacking control already emits duplicate `--spec-type` values, so no second control is needed.
