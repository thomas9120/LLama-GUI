...
- `custom_launch_args_unit.cjs`: custom launch arg tokenization, quote handling, duplicate flag warnings, and preset preservation.
  Also covers environment-variable parsing, literal/empty values, invalid and duplicate names, size limits, shared state, launch blocking, and reset. The browser smoke suite checks field placement, inline errors, preset-state restoration, and launch request construction. `test_process_environment.py` verifies backend validation, route-to-subprocess propagation, inherited values and runtime paths, and isolation between launches.
...
Starter profile coverage exercises all six context/MTP choices, repeated Auto/Off switching, preservation of sampling/model/tool settings, clearing stale runtime and draft overrides, inherited defaults across a saved-settings round trip, unlinked Auto Fit minimums and explicit Match Context, Configure checkbox default labels, shared command previews, and actual versus requested context display. Profile selection must not launch/stop a process or write saved presets.
