...
On 2026-09-12, after the Tier 2 refactor, the maintainer reported the full Windows/Chrome smoke checklist passed with model generation: populated conversation and unsent-draft transfer, both Return controls, Show window, shared sampler settings, generation with the main page minimized, Monitor updates, close/reload recovery without automatic resend, and accessible narrow-window controls in light/dark themes. Pinokio restart, Quit, relaunch and the embedded popup/fallback check also passed. This supersedes the earlier empty-workspace and popup Return-button test limits; it does not establish popup support in an embedded host that blocks opening. Public HTTPS tunnel/trusted-certificate, second-device LAN and native macOS checks remain pending; local transport results are recorded under [Local HTTPS and LAN-address follow-up](tests.md#local-https-and-lan-address-follow-up).
...
See [historical capability evidence](chat-popout-capabilities.md) and [test commands](tests.md).
