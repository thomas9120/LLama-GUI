...
- **Batch 6 complete:** stopped chat streams persist partial assistant output; Unix install/start scripts stop on Python older than 3.9; sampler Save/Import and launch-preset Import reject name collisions without writing.
- Batch 6 verification: POSIX shell syntax passed; `npm test` passed all frontend checks, 154 live llama.cpp flag checks, and the Playwright flow suite; `python -m unittest discover tests -v` passed 536 tests with 2 platform-dependent skips.
- **Batch 7 complete:** structurally malformed sampler imports are rejected atomically; app-update status-fetch failures render in the update panel; declining an HF overwrite is reported as a neutral cancellation.
- Batch 7 verification: `npm test` passed all frontend checks, 154 live llama.cpp flag checks, and the Playwright flow suite.
- **All deferred user-flow items are resolved.**
...
### [Resolved — Batch 6] Keep chat DOM and state in sync after a stopped stream (undo/regenerate removes the wrong node)
...
- **[Resolved — Batch 6] `install.sh` never checks the Python version it finds** — `install.sh:8-18` (same gap in `mac_linux_start.sh:26-30`, `mac_linux_silent_start.sh:26-30`). The backend uses 3.9+ syntax (`tuple[str, ...]` in `backend/config.py:55`); on a Python 3.8 system the install "succeeds" and the server fails at import with a raw `TypeError`.
...
- **[Resolved — Batch 7] Don't show an error when the user declines an HF overwrite** — `ui/js/hf-download-ui.js:164-174`. Cancelling the replace-confirmation falls through to `showStatus("error", "Download failed to start: ...")` — a red error for a deliberate cancel.
