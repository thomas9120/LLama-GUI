...
- Inference average speeds in Monitor and the fixed bar divide session token deltas by matching cumulative processing-time deltas from `/metrics` (`prompt_seconds_total` and `tokens_predicted_seconds_total`). Idle wall time is excluded. Restored targets and manual resets establish paired token/time baselines; a token or time counter rollback rebases the affected average. Missing counters or zero elapsed processing time display `--`; rolling gauges and live slot rates are not substituted for session averages.
