...
- Moved detected GPU cards into Monitor's shared metrics grid so a single GPU can be reordered alongside CPU, memory, disk, and inference instead of being trapped in a one-card drag container.
...
- Stabilized the frontend browser smoke test by explicitly reconciling its mocked stopped runtime instead of depending on background polling timing.
