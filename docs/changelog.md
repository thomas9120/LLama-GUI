...
- Split the 2,505-line `ui/js/presets.js` into a 10-file `ui/js/presets/` package (`presets-internal.js`, `presets-apply.js`, `presets-models.js`, `presets-local.js`, `presets-library.js`, `presets-detail.js`, `presets-roving.js`, `presets-groups.js`, `presets-crud.js`, `presets-main.js`) as pure code movement: every declaration stays a top-level script global, so behavior, evaluation order, and the public `window.LlamaGui.presets` API are unchanged, and the unit harnesses still drive module state directly. The preset, roving-focus, and launch-args harnesses now evaluate the package per file to mirror browser script boundaries.
