...
- **Cross-container moves:** out of scope for v1 — cards reorder within their own container (metrics grid, GPU grid, states, setup). Note this as a known ceiling in a `TODO:` comment; upgrade path is a single flattened grid if it's ever wanted.
