...
const FLAGS = [
    ...
    { id: "ctx_size", flag: "-c", category: "context", type: "int", label: "Total Context Window",
      ...
      desc: "Total context size in tokens. This is the maximum number of tokens the model can 'remember' at once, including your prompt, system prompt, conversation history, AND the model's reply. Larger values use more VRAM/RAM. Set 0 to use the model's default.", tool: "both", default: 16000, min: 0, max: 262144, placeholder: "16000 recommended" },
    ...
    { id: "fit", flag: "-fit", category: "gpu", submenu: "Auto Fit", type: "enum", label: "Auto Fit to VRAM",
      short_desc: "Automatically adjusts unset memory-heavy settings so the model is less likely to crash from running out of VRAM.",
      beginner_tip: "Keep this ON unless you specifically need manual control.",
      desc: "Main auto-fit switch. When ON, llama.cpp may lower unset values like context and batch size so the model can fit in available GPU memory. This does not replace the helper settings below: -fitt sets how much free VRAM to leave as safety margin, and -fitc sets the minimum context floor auto-fit should respect. If you turn -fit OFF, those helper settings no longer do anything.", tool: "both",
      default: "on",
      options: [{ value: "on", label: "On (Recommended)" }, { value: "off", label: "Off" }] },
    { id: "fit_target", flag: "-fitt", category: "gpu", submenu: "Auto Fit", type: "text", label: "Fit Target Margin",
      short_desc: "Tells auto-fit how much VRAM to leave unused as breathing room on each device.",
      beginner_tip: "Leave this alone unless auto-fit is still too aggressive for your system.",
      desc: "Safety margin for auto-fit, in MiB. Higher numbers make auto-fit more conservative by leaving more free VRAM; lower numbers let it pack the model more tightly. This does not override -fit or -fitc: it only guides -fit when -fit is ON. Accepts one value for all devices, or a comma-separated list like 1024,2048 for multi-GPU setups.", tool: "both",
      default: "1024", placeholder: "1024 or 1024,2048" },
    { id: "fit_ctx", flag: "-fitc", category: "gpu", submenu: "Auto Fit", type: "int", label: "Minimum Auto Fit Context",
      short_desc: "Sets the smallest context window auto-fit is allowed to fall back to.",
      beginner_tip: "Raise this only if you need a guaranteed minimum context and have enough VRAM to support it.",
      desc: "Minimum context size that auto-fit is allowed to choose. Think of this as a floor, not a replacement for your normal context setting: -fit still decides whether to adjust memory-related settings, while -fitc only limits how far context may be reduced during that process. If -fit is OFF, this setting has no effect.", tool: "both",
      default: 16000, min: 0, max: 262144, placeholder: "16000" },
    ...
    { id: "xtc_probability", flag: "--xtc-probability", category: "sampling", type: "float", label: "XTC Probability",
      desc: "XTC token removal probability (0 = disabled)", tool: "both", min: 0, max: 1, step: 0.01 },
    { id: "xtc_threshold", flag: "--xtc-threshold", category: "sampling", type: "float", label: "XTC Threshold",
      desc: "XTC threshold (1.0 = disabled)", tool: "both", min: 0, max: 1, step: 0.01 },
    ...
    { id: "mirostat_lr", flag: "--mirostat-lr", category: "sampling", type: "float", label: "Mirostat LR",
      desc: "Mirostat learning rate (eta)", tool: "both", min: 0.001, max: 1, step: 0.01 },
    { id: "mirostat_ent", flag: "--mirostat-ent", category: "sampling", type: "float", label: "Mirostat Entropy",
      desc: "Mirostat target entropy (tau)", tool: "both", min: 0, max: 20, step: 0.1 },
